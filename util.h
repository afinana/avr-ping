...
// FIXME: all this LED feedback stuff has it's own module now, it should
...
// FIXME: we should perhaps use a static buffer for the message, to reassure
// people who like to freak out about dynamic allocation on microcontrollers.
// Then again perhaps people who like to freak out about a malloc right before
// an assert(0) because they might be within 40 bytes of running out of memory
// should just be allowed to freak out, since the alternatives are to put a
// definitionin in a header, or add a C file where one is otherwise not needed
