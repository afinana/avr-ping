...
// (i.e. no CTS/RTS or other extra serial port signals).  In general this
// interface isn't safe to use from interrupts handlers.
...
// digital IO.  The ATMega328P datasheet says that UART0 must be reinitialized
...
// Send a byte to the serial port.  The TXC0 bit of UCSR0A is cleared
// (oddly writing a one to it clears it) so that we can determine later
// whether transmission of the byte has completed.  This has to be done
// manually because we aren't using interrupts.
...
// Evaluate to true iff everything written to the transmit buffer (via
// UART_PUT_BYTE()) so far has been "shifted out" (i.e. completely sent).
#define UART_TRANSMIT_COMPLETE() (UCSR0A & _BV (TXC0))

// Block until everything written to the transmit buffer (via UART_PUT_BYTE()
// so far as been "shifted out" (i.e. completely sent).
#define UART_WAIT_FOR_TRANSMIT_COMPLETE() loop_until_bit_is_set (UCSR0A, TXC0)
...
// block forever.  You should check for errors before actually reading the
// byte, since you can't do so afterwords.
